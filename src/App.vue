...
export default {
  ...
  methods: {
    addTicker() {
      ...
      setTimeout(async function dataFetcher() {
        const response = await fetch(
          `https://min-api.cryptocompare.com/data/price?fsym=${valueName}&tsyms=USD&api_key=42b0663186988e084e85c51390423c69b58d7d953e7139a9cf12b461706d43c9`,
        );
        const newValue = await response.json();

        if (newValue.Message === 'Error') {
          ctx.removeTicker(valueName);
          return;
        }

        const tickerFromArray = ctx.tickers.find(({ name }) => name === valueName);

        if (!tickerFromArray) return;

        tickerFromArray.value =
          newValue.USD > 1 ? newValue.USD.toFixed(3) : newValue.USD.toPrecision(3);

        if (ctx.activeTicker?.name === valueName) {
          ctx.graph.push(newValue.USD);
        }

        setTimeout(dataFetcher, 5000);
      }, 5000);
